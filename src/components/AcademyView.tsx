...
export const AcademyView: React.FC<AcademyViewProps> = ({
  ...
  const [viewMode, setViewMode] = useState<'overview' | 'lesson'>(
    initialLessonId ? 'lesson' : 'overview'
  );
  const [selectedStageId, setSelectedStageId] = useState<number>(initialStageId || user.currentStageId || 1);
  ...
  const handleAskLessonAi = async (promptText?: string) => {
    ...
    try {
      const res = await fetch('/api/ask-gommar', {
        method: 'POST',
          ...
          {localizedStages.map((stage) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-3.5 py-2.5 rounded-xl border text-xs font-bold transition-all duration-200 cursor-pointer ${
                  isStageActive
                    ? 'bg-slate-900 dark:bg-slate-800 text-white border-indigo-600 dark:border-indigo-500 shadow-md scale-105'
                    : isStageFullyDone
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 space-y-8 shadow-sm">
            ...
            {activeLessonTab === 'handbook' && (
              <div className="space-y-6 animate-fadeIn">
                ...
                {language !== 'de' && currentStage.id > 8 && currentLesson.learnContent.fullArticleGuide && (
